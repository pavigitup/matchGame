...
class App extends Component {
  state = {
    ...
    filteredImagesList: initialImagesList,
    tabId: tabsList[0].tabId,
    timerLimit: 60,
    selectedThumbnailUrl: initialImagesList.find(
      item => item.category === 'FRUIT',
    ).thumbnailUrl, // Initialize with the first fruit thumbnail
    score: 0,
  }

  filteredLists = () => {
    const {filteredImagesList, tabId} = this.state
    const onlyFruits = filteredImagesList.filter(
  ...
  getRandomImage = () => {
    const {filteredImagesList} = this.state
    const randomIndex = Math.floor(Math.random() * filteredImagesList.length)
    return filteredImagesList[randomIndex]
  }

  getTabList = selectedTabId => {
    const {imagesList} = this.state
    const filteredList = imagesList.filter(
      eachTab => eachTab.category === selectedTabId,
    )

    this.setState(
      {filteredImagesList: filteredList, tabId: selectedTabId},
      () => {
        this.filteredLists()
      },
    )
  ...
  fruitId = id => {
    const selectedImage = initialImagesList.find(eachTab => eachTab.id === id)
    ...
    this.setState({
      selectedThumbnailUrl: randomImage.thumbnailUrl,
    })
  }

  render() {
    const {timerLimit, selectedThumbnailUrl, score} = this.state
    const filteredImagesList = this.filteredLists()
        ...
        <div className="navbar">
          ...
          <div className="score-time-con">
            ...
            <div className="timer-con">
              ...
              />
              <p className="timer-para">{timerLimit} sec</p>
            </div>
          </div>
        </div>
        <div className="game-con">
          <ul>
            {/* Display selected thumbnail in NavItems */}
            <NavItems thumbnailUrl={selectedThumbnailUrl} />
          </ul>
          <ul className="selection-tag">
